...
from aiogram.types import Message, FSInputFile, ReplyKeyboardMarkup, InlineKeyboardMarkup, KeyboardButton, \
...
from bot.checkout import generate_liqpay_url
...
async def cmd_start(message: Message):
    photo_path = "D:/Bachelor/static/image/first_photo.jpg"
    photo = FSInputFile(photo_path)  # Використання FSInputFile для локального файлу
    await message.answer_photo(photo=photo, caption="Привіт! Радий бачити тебе в одній з "
...
async def menu_all(message: Message):
    ...
    await message.answer_photo(photo=photo, caption="Обирай що саме сьогодні припаде тобі до душі",
...
async def send_item_details(callback_query: CallbackQuery, model_class, item_type: str):
    try:
        ...
        if item_type not in ["pizza", "drink", "sauce"]:
            ...
            await callback_query.answer("Невідома категорія. Спробуйте ще раз.", show_alert=True)
            return

        if len(data_parts) < 3:
            await callback_query.answer("Невірний формат даних. Спробуйте ще раз.", show_alert=True)
            return

        item_id = int(data_parts[2])
        item = await sync_to_async(model_class.objects.get)(id=item_id)  # Отримуємо об'єкт товару
        ...
        keyboard = InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    InlineKeyboardButton(text="❌", callback_data=f"remove_{item_type}_{item_id}"),
                    InlineKeyboardButton(text="➖", callback_data=f"decrease_{item_type}_{item_id}"),
                    InlineKeyboardButton(text="➕", callback_data=f"increase_{item_type}_{item_id}"),
                ],
                [
                ...
                [
                    InlineKeyboardButton(text="<< Назад", callback_data=f"back_to_menu_{item_type}"),
                    InlineKeyboardButton(text=f"<< Кошик ({cart_total} грн)", callback_data="view_cart")
                ]
            ]
        )
...
def get_or_create_cart(telegram_id):
    ...
    cart = Cart.objects.filter(is_active=True, user=user).first()
    if not cart:
...
async def handle_increase(callback_query: types.CallbackQuery, state):
    ...
    await handle_callback(
        callback_query, state, cart=cart, model_classes={
            "pizza": PizzaList,
            "drink": DrinkList,
            "sauce": SauceList
        }
...
def update_cart_total(cart_id):
    try:
        cart, created = Cart.objects.get_or_create(id=cart_id, defaults={"total_sum": 0})
    except Exception as e:
        logger.error(f"❌ Помилка при отриманні кошика: {e}")
        return

    cart_items = CartItem.objects.filter(carts=cart)
...
async def handle_add_to_cart(callback_query: CallbackQuery, state):
    try:
        ...
        current_quantity = state_data.get(f"{item_type}_{item_id}_quantity", 1)  # Якщо немає - ставимо 1
        ...
        cart_item, created = await sync_to_async(lambda: CartItem.objects.get_or_create(
            category=item_type,
            item_id=item_id,
            defaults={
                "title": item.title,
        ...
        new_cart_text = f"<< Кошик ({cart_total} грн)" if cart_total > 0 else "<< Кошик (0 грн)"
        ...
        await callback_query.answer("❌ Помилка при додаванні в кошик!", show_alert=True)
...
async def handle_callback(callback_query: CallbackQuery, state, cart, model_classes):
    try:
        ...
        state_data = await state.get_data()
        current_quantity = state_data.get(f"{item_type}_{item_id}_quantity", 1)  # Значення за замовчуванням: 1

        # Логіка дій
        if action == "increase":
            current_quantity += 1
            await update_cart_total(cart.id)  # Оновлюємо суму кошика
            cart_total = await sync_to_async(lambda: cart.total_sum)()  # Отримуємо нову суму
...
async def back_to_menu(callback_query: CallbackQuery):
    try:
        ...
        if item_type == "pizza":
            ...
            await callback_query.answer(f"Невідома категорія. Спробуйте ще раз. {item_type}", show_alert=True)
            return

        cart = await sync_to_async(Cart.objects.get)(id=callback_query.from_user.id)
...
async def get_item_price(item_type: str, item_id: int) -> int:
    ...
    try:
        item = await sync_to_async(model.objects.get)(id=item_id)  # Виконуємо ORM-запит у окремому потоці
        return item.price
...
async def reset_quantity(callback: CallbackQuery, state):
    try:
        ...
        for row in keyboard:
            for button in row:
                if button.callback_data.startswith(f"add_to_cart_{item_type}_{item_id}"):
...
async def get_cart_items():
    items = await sync_to_async(list)(CartItem.objects.all())
    return {str(item.id): {"name": item.title, "quantity": item.quantity, "price": item.price, "category": item.category, "item_id": item.item_id} for item in items}
...
def generate_cart_keyboard(cart_items, cart_total):
    ...
    for item_id, item in cart_items.items():
        keyboard_buttons.append([
            InlineKeyboardButton(text=f"{item['name']} - {item['quantity']} шт. {item['price']} грн",
        ...
        logger.debug(f"✅ Згенеровано callback_data для редагування: {callback_data_edit}")
        logger.debug(f"🔍 Генерація callback_data: category={category}, item_id={item_id}")

        keyboard_buttons.append([
            InlineKeyboardButton(text="Змінити ✏", callback_data=callback_data_edit),
            InlineKeyboardButton(text="Видалити ⛔", callback_data=callback_data_delete)
        ])

    if keyboard_buttons:
        keyboard_buttons.append([InlineKeyboardButton(text="🗑 Оформити", callback_data="checkout")])
        keyboard_buttons.append([InlineKeyboardButton(text="🚫 Відмінити", callback_data="cancel")])
...
async def show_cart(callback_query: CallbackQuery):
    ...
    cart_total = sum(item['price'] * item['quantity'] for item in cart_items.values()) if cart_items else 0
    keyboard = generate_cart_keyboard(cart_items, cart_total)
    text = f"🛒 Ваше замовлення:\n_Загальна сума: {cart_total} грн_" if cart_items else "🛒 Ваш кошик порожній."
...
async def edit_item(callback_query: CallbackQuery):
    ...
    cart_item = await sync_to_async(lambda: CartItem.objects.filter(category=category, item_id=item_id).first())()
    if not cart_item:
    ...
    await sync_to_async(cart_item.save)()
    cart_total = await sync_to_async(lambda: sum(item.price * item.quantity for item in CartItem.objects.all()))()
    ...
    keyboard = InlineKeyboardMarkup(
        inline_keyboard=[
            [InlineKeyboardButton(text="➖", callback_data=f"edit_decrease_{category}_{item_id}"),
             InlineKeyboardButton(text="➕", callback_data=f"edit_increase_{category}_{item_id}")],
            [InlineKeyboardButton(text=f"✅ Зберегти", callback_data=f"edit_confirm_{category}_{item_id}")],
            [InlineKeyboardButton(text=f"<< Кошик ({cart_total} грн)", callback_data="view_cart")]
        ]
    )
    ...
    if current_text != new_text or current_markup != keyboard:
        await callback_query.message.edit_text(new_text, reply_markup=keyboard, parse_mode="Markdown")
    else:
...
async def delete_item(callback_query: CallbackQuery):
    ...
    await sync_to_async(lambda: CartItem.objects.filter(category=category, item_id=item_id).delete())()
